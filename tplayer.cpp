...
TPlayer::TPlayer(WId winid)
{
    ...
    // Set output media
    play_obj.play_select = SELECT_VIDEO;
    play_obj.play_idx = 0;
...
gboolean TPlayer::play_image(gpointer user_data)
{
    PipeElement * obj = (PipeElement *)user_data;

    if((obj->state == STATE_PLAYING)&&(obj->play_select == SELECT_VIDEO))
    {
        obj->state = STATE_STOP;
        obj->play_idx = 0;
...
gboolean TPlayer::play_video(gpointer user_data)
{
    PipeElement * obj = (PipeElement *)user_data;

    if((obj->state == STATE_PLAYING)&&(obj->play_select == SELECT_IMAGE))
    {
        obj->state = STATE_STOP;
        obj->play_idx = 0;
